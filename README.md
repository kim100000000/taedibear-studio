...
AI가 짠 코드를 그대로 쓰지는 않았습니다. 별도로 보안 점검을 진행해 `docs/08_보안품질점검보고서.md`에 Critical 8건(C1~C8)을 정리했고, 결제 금액 서버 검증, CORS 오리진 제한, JWT 관련 노출 제거, SSRF 방어 등을 하나씩 반영하며 커밋에 남겼습니다. AI를 코드 생성 도구가 아니라 기획·구현·리뷰 전 과정의 작업 파트너로 쓰고, 최종 검증과 책임은 직접 지는 방식으로 개발했습니다.
